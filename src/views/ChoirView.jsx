...
import { Music, ChevronDown, Users, Quote } from 'lucide-react';
import RevealOnScroll from '../components/ui/RevealOnScroll';
import { choirInstruments } from '../data/mockData';
import pcoro from '../assets/img/pcoro.jpg';
      ...
      <div className="max-w-4xl mx-auto text-center mb-16 relative">
        ...
        <RevealOnScroll direction="down">
          ...
          <p className="text-lg text-stone-600 dark:text-stone-300 max-w-2xl mx-auto leading-relaxed">
            Un grupo apasionado de adoradores unidos por el propósito de exaltar el nombre de Cristo a través de la música.
          </p>
        </RevealOnScroll>
      </div>
          ...
          <div className="bg-white dark:bg-[#1e1a17] rounded-3xl overflow-hidden shadow-lg border border-stone-100 dark:border-stone-800 mb-20 flex flex-col md:flex-row">
            ...
            <div className="p-8 md:p-12 flex flex-col justify-center">
              ...
              <p className="text-stone-600 dark:text-stone-300 leading-relaxed mb-6">
                Bajo la dirección del Pastor Isaías, nuestro coro ha crecido no solo en técnica vocal, sino en sensibilidad espiritual. Creemos que la adoración no es solo cantar, es una ofrenda de vida.
              </p>
        ...
        <div className="mb-20">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 items-start">
            {choirInstruments.map((item, idx) => {
              ...
              return (
                <RevealOnScroll key={item.id || `instrument-${idx}`} direction="up" delay={idx * 100}>
                  <div
                    onClick={() => toggleInstrument(idx)}
                    className={`bg-white dark:bg-[#1e1a17] rounded-2xl border transition-all duration-500 overflow-hidden cursor-pointer group ${isExpanded
                        ? 'border-[#3D6599] shadow-xl ring-2 ring-[#3D6599]/20 col-span-1 md:col-span-2 lg:col-span-3'
                        : 'border-stone-100 dark:border-stone-800 shadow-sm hover:border-[#C7DBEB] dark:hover:border-[#3D6599] hover:shadow-md'
                      }`}
                  >
                    <div className="p-6 flex items-start justify-between">
                      <div className="flex items-center gap-4">
                        <div className={`w-12 h-12 rounded-full flex items-center justify-center transition-colors duration-300 ${isExpanded ? 'bg-[#3D6599] text-white' : 'bg-[#C7DBEB]/30 dark:bg-[#3D6599]/20 text-[#3D6599] dark:text-[#C7DBEB]'
                          }`}>
                          {item.icon}
                        </div>
                        <div>
                      ...
                      <div className="p-6 pt-0 border-t border-dashed border-stone-200 dark:border-stone-800 mt-2 flex flex-col md:flex-row gap-8">
                        <div className="md:w-1/3 flex-shrink-0 flex flex-col gap-4">
                          <div className="flex items-center gap-2 mb-1">
                            <Users size={14} className="text-[#3D6599]" />
                          ...
                          <div className={`grid gap-4 ${item.responsibles.length > 1 ? 'grid-cols-2' : 'grid-cols-1'}`}>
                            {item.responsibles.map((resp, rIdx) => (
                              <div key={resp.id || `resp-${rIdx}`} className="group/photo">
                                <div className="rounded-xl overflow-hidden aspect-square shadow-md border border-stone-100 dark:border-stone-700 relative w-full max-w-[90px] sm:max-w-[110px] md:max-w-[130px] lg:max-w-[150px] mx-auto">
                                  <img
                                    ...
                                    draggable="false"
                                    onContextMenu={(e) => e.preventDefault()}
                                    className="w-full h-full object-cover transition-transform duration-500 group-hover/photo:scale-110 select-none"
                                  />
                                ...
                                <p className="text-xs font-bold text-center mt-2 text-stone-700 dark:text-stone-200">{resp.name}</p>
                              </div>
                            ))}
                          </div>
                        </div>
                        ...
                        <div className="md:w-2/3 flex flex-col justify-center border-l border-stone-100 dark:border-stone-800 pl-0 md:pl-8">
                          <div className="mb-6 relative">
                            <Quote className="absolute -top-3 -left-3 text-[#C7DBEB] dark:text-[#3D6599]/30 transform -scale-x-100" size={48} />
                          ...
                          <div className="flex flex-wrap gap-3 mt-auto">
                            <span className="text-xs font-bold text-stone-500 dark:text-stone-400 bg-stone-100 dark:bg-stone-800 px-4 py-2 rounded-full flex items-center gap-2">
                              <Users size={14} className="text-[#3D6599]" /> {item.count} Músicos Activos
                            </span>
                          </div>
                        </div>
                      </div>
